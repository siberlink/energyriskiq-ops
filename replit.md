...
- **EERI SEO Infrastructure:** Public EERI pages at `/eeri`, `/eeri/methodology`, `/eeri/history`, `/eeri/{date}`, `/eeri/{year}/{month}`. Shows 24h delayed data publicly with: level, band, trend, interpretation, top 3 drivers, affected assets, methodology hints. All EERI pages automatically included in sitemap.xml and sitemap.html. Service layer: `src/reri/eeri_history_service.py`, SEO routes: `src/reri/seo_routes.py`.
