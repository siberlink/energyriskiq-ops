...
- **Database:** PostgreSQL is used for persistence. Dual-database architecture: `DATABASE_URL` for app data (users, sessions, plans), `PRODUCTION_DATABASE_URL` for production intelligence data (indices, alerts, assets). All user-facing intelligence reads use `execute_production_query`/`get_production_cursor` from `src/db/db.py` to always show the latest production data, even in the dev preview. Write operations (computation engines, SEO page generation) use the default `get_cursor`/`execute_query`.
