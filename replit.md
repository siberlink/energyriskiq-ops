...
- **EERI Pro Dashboard Module:** Provides Pro-tier users with real-time EERI display, component breakdown, asset stress panel, top risk drivers, historical intelligence, regime statistics, and daily AI-generated summaries. Trader tier (plan_level ≥ 2) includes 9 tactical intelligence modules: reaction lag analysis, 30-day rolling Pearson correlations, regime persistence probability (computed from historical band transitions), risk shock detector (Δ≥+10 in 2 days), key energy market dates, data timestamps, asset impact ranking (sensitivity = |weekly_move_pct| × 30d_correlation), and one-line trading insight. API: `GET /api/v1/eeri-pro/trader-intel`. Frontend uses `.eeri-trader-only` CSS class for visibility gating. ERIQ knowledge base includes `ERIQ/eeri-trader-dashboard.md` (9 chunks, 205 total chunks from 7 files).
- **GERI Trader Dashboard Module:** Provides Trader+ tier users (plan_level ≥ 2) with 7 tactical intelligence modules: risk regime status (escalating/de-escalating/stable), cross-asset confirmation score (0-100), divergence indicator (per-asset alignment), lead/lag intelligence (cross-correlation timing), EU gas storage seasonal context (vs 5-year norms), asset reaction summary (AI narrative), and alert preview (latest 5 by severity). API: `GET /api/v1/indices/geri/trader-intel`. Frontend loads asynchronously via `loadGeriTraderIntel()` into `#geriTraderIntelSection`. ERIQ knowledge base includes `ERIQ/geri-trader-dashboard.md`. Files: `src/geri/trader_intel.py` (computations), `src/geri/routes.py` (API route).
...
- **ERIQ Expert Analyst:** AI-powered interpretation intelligence bot accessible only at authenticated `/users/account` page. Uses GPT-5.1 via direct OPENAI_API_KEY (max_completion_tokens, no temperature param) with automatic fallback to gpt-4.1-mini via platform proxy if direct key unavailable/quota exceeded. RAG from `/ERIQ/*.md` knowledge base (205 chunks from 7 methodology documents). Product Analytics & Feedback Layer: structured feedback tags (helpful, not_helpful, inaccurate, etc.), internal analytics endpoint (`GET /api/v1/eriq/analytics` secured by INTERNAL_RUNNER_TOKEN) tracking top questions, low-satisfaction responses, intent/mode distribution, plan usage, tag distribution, daily volume, 30-day summary. Analytics insights fed into CAL context for self-improvement. Context Assembly Layer (CAL) reads from production database via `execute_production_query` (uses `PRODUCTION_DATABASE_URL` env var if set, falls back to `DATABASE_URL`). CAL provides read-only, plan-gated database snapshots across all 10 integration domains: index engine outputs (GERI as `global:geo_energy_risk`, EERI as `europe:eeri` with full JSONB components/drivers parsing), alert intelligence, asset intelligence (Brent/WTI/TTF/VIX/EUR-USD/EU storage), methodology knowledge base (196 RAG chunks), regime classification, correlations/betas (rolling 14-day/30-day), data quality assessment, historical patterns, and plan-based entitlements. Free tier gets 24h-delayed data, 3 q/day (Explain only). Personal: realtime + 15 q/day + Interpret. Trader: 60 q/day + Decide-support + drivers/regime. Pro: 200 q/day + full suite. Enterprise: unlimited. EGSI tables use `index_date` column and `components_json` JSONB. Files: `src/eriq/context.py` (CAL), `src/eriq/knowledge_base.py` (RAG), `src/eriq/router.py` (intent routing), `src/eriq/agent.py` (answer generation), `src/eriq/tokens.py` (token economy), `src/api/eriq_routes.py` (API endpoints). API: POST `/api/v1/eriq/ask`, GET `/api/v1/eriq/status`, POST `/api/v1/eriq/feedback`, GET `/api/v1/eriq/history`, GET `/api/v1/eriq/tokens/status`, POST `/api/v1/eriq/tokens/checkout`.
- **ERIQ Token Economy:** Plan-based monthly allowances reset on Stripe subscription payment (Personal: 100K, Trader: 300K, Pro: 500K, Enterprise: 1M tokens). Two-tier balance: monthly allowance (resets on invoice.paid webhook) + purchased_balance (never resets). Deduction prioritizes allowance first, then purchased. Token purchases via Stripe one-time payment at 1.35 EUR per 100K tokens (packs: 100K/300K/500K/1M). Webhook idempotency via ledger ref_info checks. Low balance warning at 3000 tokens. Tables: `eriq_token_balances`, `eriq_token_ledger`. Token allowance also granted on initial checkout.session.completed for new subscriptions.
...
## Testing Credentials
- **Test User (Trader Plan):** emil.siberlink@gmail.com / Password: Regen@3010 / PIN: 221967