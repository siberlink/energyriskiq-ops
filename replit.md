...
- **ERIQ Expert Analyst:** AI-powered interpretation intelligence bot accessible only at authenticated `/users/account` page. Uses GPT-4.1-mini with RAG from `/ERIQ/*.md` knowledge base (196 chunks from 6 methodology documents). Context Assembly Layer (CAL) provides read-only, plan-gated database snapshots including all indices, alerts, assets, regime, correlations, and betas. Plan-tiered access: Free (3 q/day, Explain only), Personal (15 q/day, +Interpret), Trader (60 q/day, +Decide-support), Pro (200 q/day, full suite), Enterprise (unlimited). Intent router classifies questions into methodology, explain_today_move, cross_index_compare, divergence_explain, scenario_explain, asset_impact, alert_explain, regime_explain, and greeting intents. Guardrails prevent financial advice. Conversations logged in `eriq_conversations` table. Files: `src/eriq/context.py` (CAL), `src/eriq/knowledge_base.py` (RAG), `src/eriq/router.py` (intent routing), `src/eriq/agent.py` (answer generation), `src/api/eriq_routes.py` (API endpoints). API: POST `/api/v1/eriq/ask`, GET `/api/v1/eriq/status`, POST `/api/v1/eriq/feedback`, GET `/api/v1/eriq/history`.
