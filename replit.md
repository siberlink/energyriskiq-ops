...
- **EERI Weekly Snapshot (Public):** Embedded section on `/eeri` page between Risk Level Bands and Daily Intelligence Narrative. 7 blocks: Weekly Risk Overview (dark card), Risk Regime Distribution, Cross-Asset Confirmation Panel (TTF Gas, Brent Oil, VIX, EUR/USD, EU Gas Storage with Confirming/Neutral/Diverging labels), Mini Weekly Overlay Charts (Chart.js indexed from 100), Divergence Watch Badge, Historical Context Box, Next-Week Historical Tendencies. Service: `src/reri/eeri_weekly_snapshot.py`. Uses most recently completed ISO week data. See `docs/eeri-weekly-public-snapshot.md` for design spec.
