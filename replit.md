...
3. **NO DELAY ON PUBLIC INDEX CARDS.** The landing page (`/`) and all public index pages now show the **latest** (LIMIT 1, no OFFSET) value from production. Do NOT apply OFFSET 1 to GERI, EERI, or EGSI on public-facing pages. The landing page section cards (GERI/EERI/EGSI) are populated via `/api/hero-snapshot` which queries `LIMIT 1` from each table.
   - **GERI** → `SELECT value, band, trend_7d FROM intel_indices_daily WHERE index_id='global:geo_energy_risk' ORDER BY date DESC LIMIT 1`
   - **EERI** → `SELECT value, band, trend_7d FROM reri_indices_daily WHERE index_id='europe:eeri' ORDER BY date DESC LIMIT 1`
   - **EGSI-M** → `SELECT index_value AS value, band, trend_7d FROM egsi_m_daily WHERE region='Europe' ORDER BY index_date DESC LIMIT 1`
...
- **JKM LNG Spot Price Page:** A SEO-optimised public page at `/data/jkm-lng-spot-price` showing the Japan Korea Marker LNG spot price with server-side SVG charts (7D/30D/90D/All period toggles), sortable/searchable/paginated historical data table, FAQ accordion, risk context (GERI/EERI/EGSI), benchmark comparison cards, full link wheel to all `/data/` pages, and CSV download at `/api/jkm-lng-spot-price.csv`. Data from `lng_price_snapshots` (production DB). Route file: `src/api/jkm_routes.py`. Registered in `src/api/app.py`.
